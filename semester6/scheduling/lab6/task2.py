def main():
    ...
    for job_duration in job_durations:
        ...
        for i in range(len(assigned_jobs[min_worker_index])):
            assigned_jobs[min_worker_index][i] += (
                job_duration * worker_speeds[min_worker_index]
            )

        assigned_jobs[min_worker_index].insert(
            0, (job_duration * worker_speeds[min_worker_index])
        )
        worker_times[min_worker_index] = (
            min_worker_time + worker_speeds[min_worker_index]
        )
    total_time = 0
