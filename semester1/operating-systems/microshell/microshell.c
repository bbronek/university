...
static void change_directory(int count, char **arguments) {
    ...
    const char *destination = count == 1 || strcmp(arguments[1], "~") == 0 ? getenv("HOME")
...
int main(void) {
    ...
    for (;;) {
        if (interactive) {
            ...
            printf("%s $ ", getcwd(directory, sizeof directory) == NULL ? "?" : directory);
            fflush(stdout);
        }
        if (getline(&line, &capacity, stdin) == -1)
            break;
        int count = parse_arguments(line, arguments, sizeof arguments / sizeof arguments[0]);
        if (count < 0) {
